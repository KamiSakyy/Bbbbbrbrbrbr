...
Последний релиз: **[Releases → Яндекс Браузер](https://github.com/KamiSakyy/Bbbbbrbrbrbr/releases/latest)**

| Файл | Размер | Кому |
|------|--------|------|
| `app-arm64-v8a-release.apk` | ~88 МБ | почти все телефоны с 2017 года |
| `app-armeabi-v7a-release.apk` | ~85 МБ | старые 32-битные устройства |
| `app-x86_64-release.apk` | ~93 МБ | эмуляторы, планшеты на Intel |

Скачайте APK на телефон, откройте его и разрешите установку из неизвестных источников.
Целостность проверяется файлом `SHA256SUMS.txt` из того же релиза.
...
### Локально
```bash
# Нужен JDK 21 и Android SDK (compileSdk 37, AGP 9.1.1, Gradle 9.3.1)
...
Фактические цифры релиза (arm64-v8a, замерено на сборке):

| Что | До оптимизаций | После |
|-----|----------------|-------|
| APK | 183 МБ | **88 МБ** |
| из них `libxul.so` (движок) | 152 МБ | ~62 МБ в сжатом виде |
| `assets/omni.ja` (ресурсы движка) | 15 МБ | 15 МБ |
| `classes.dex` (весь UI на Compose) | 4.7 МБ | 4.7 МБ |

Что включено:

- **R8** (полная обфускация + удаление мёртвого кода) и **shrinkResources**.
- **Сжатие нативных библиотек** (`packaging.jniLibs.useLegacyPackaging = true`): система
  распаковывает `.so` при установке, APK для скачивания уменьшается в ~2.5 раза.
- Раздельные APK по ABI — каждый содержит только движок своей архитектуры.
- Только ресурсы `ru` + `en`.
- Никаких лишних библиотек: ~30 своих векторных иконок вместо `material-icons-extended` (~2 МБ),
  свой загрузчик фавиконок с дисковым кэшем вместо Coil/Glide, свой тост и своя шторка меню.

Столько весит сам движок: `libxul.so` — это Gecko целиком (Blink-уровня движок с V8-подобным
SpiderMonkey), а не надстройка над системным WebView. Уменьшить это можно только отказом от
собственного движка, чего мы сознательно не делаем.
